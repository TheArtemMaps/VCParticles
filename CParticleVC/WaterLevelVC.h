...
{
public:
	static int32_t       ms_nNoOfWaterLevels;
	static float       ms_aWaterZs[48];
	static CRect       ms_aWaterRects[48];
	static int8_t        aWaterBlockList[MAX_LARGE_SECTORS][MAX_LARGE_SECTORS];				// 64x64 Large blocks 64x64 each
	static int8_t        aWaterFineBlockList[MAX_SMALL_SECTORS][MAX_SMALL_SECTORS];			// 128x128 Small blocks 32x32 each
	...
	static int32_t       m_nRenderWaterLayers;
	...
	static bool    WaterLevelAccordingToRectangles(float fX, float fY, float* pfOutLevel = NULL);
...
#else
	static void RenderWavyMask(float fX, float fY, float fZ, float fSectorX, float fSectorY, int32_t nCamDirX, int32_t nCamDirY, RwRGBA const& color);
#endif
